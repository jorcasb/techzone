...
<body>
  ...
  <section class="intro">
  <div class="container">
  <form action="formularios/EditarDatos.php" method="POST">
        <?php
        ...
        <select class="form-select mb-3" aria-label="Default select example" name="Info">
            <option selected disabled>--Seleccione Productos--</option>
            <?php
                include ("conexion.php");

                $sql3 = "SELECT * FROM info WHERE idInfo=".$row['InfoId'];
                $resultado3 = $conexion->query($sql3);

                $row3 = $resultado3->fetch_assoc();

                echo "<option selected value='".$row3['idInfo']."'>".$row3['NombreInfo']."</option>";

                $sql4 = "SELECT * FROM info";
                $resultado4 = $conexion->query($sql4);

                while ($Fila = $resultado4->fetch_array()) {
                    echo "<option value='".$Fila['idInfo']."'>".$Fila['NombreInfo']."</option>";
                }
            ?>   
        </select>
        ...
        <div class="text-center">
            ...
            <a href="get-a-quote.php" class="btn btn-dark">Regresar</a>
        </div>
    </form>
    </div>
</section>
